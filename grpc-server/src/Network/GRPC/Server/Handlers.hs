...
type ServerStreamHandler i o a =
  Request -> i -> IO (a, ServerStream o a)

-- * Client streaming -----------------------------------------------------------

data ClientStream i o a
  = ClientStream
...
type ClientStreamHandler i o a =
  Request -> IO (a, ClientStream i o a)
...
type BiDiStreamHandler i o a =
  Request -> IO (a, BiDiStream i o a)
...
unary ::
  ...
  ServiceHandler
unary rpc handler =
  HM.singleton (path rpc) (handleUnary rpc handler)
...
serverStream rpc handler =
  HM.singleton (path rpc) (handleServerStream rpc handler)
...
clientStream rpc handler =
  HM.singleton (path rpc) (handleClientStream rpc handler)
...
bidiStream rpc handler =
  HM.singleton (path rpc) (handleBiDiStream rpc handler)
...
generalStream rpc handler =
  HM.singleton (path rpc) (handleGeneralStream rpc handler)
...
handleUnary rpc handler decoding encoding req write flush =
  handleRequestChunksLoop
    (decodeInput rpc $ _getDecodingCompression decoding)
    handleMsg
    handleEof
    nextChunk
  where
...
handleGeneralStream ::
  ...
  GeneralStreamHandler i o a b ->
  WaiHandler
handleGeneralStream rpc handler0 decoding encoding req write flush =
  void $
    handler0 req >>= go
  where
    ...
    nextChunk = getRequestBodyChunk req
    reply msg = write (encodeOutput rpc (_getEncodingCompression encoding) msg) *> flush
    go (in0, instream, out0, outstream) =
      concurrently
...
handleRequestChunksLoop decoder handleMsg handleEof nextChunk =
  case decoder of
    ...
    (Done _ _ (Left err)) ->
      closeEarly (GRPCStatus INVALID_ARGUMENT (BC8.pack $ "done-error: " ++ err))
    (Fail _ _ err) ->
      closeEarly (GRPCStatus INVALID_ARGUMENT (BC8.pack $ "fail-error: " ++ err))
    partial@(Partial _) -> do
      chunk <- nextChunk
      if BC8.null chunk
        then handleEof
        else handleRequestChunksLoop (pushChunk partial chunk) handleMsg handleEof nextChunk
