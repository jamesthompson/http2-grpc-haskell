...
instance Exception UnallowedPushPromiseReceived
...
instance Exception StreamReplyDecodingError
...
streamReply rpc v0 req handler = RPCCall rpc $ \conn stream isfc osfc encoding decoding ->
  let loop v1 decode hdrs = _waitEvent stream >>= \case
...
sendSingleMessage rpc msg encoding flagMod conn connectionFlowControl stream streamFlowControl = do
  ...
  let goUpload dat = do
        ...
        if got == wanted
          then sendData conn stream flagMod dat
          else do
...
instance Exception InvalidParse
